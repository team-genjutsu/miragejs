  ...
  navigator.getMedia({
    ...
    socket.on('initiated', (chatter) => {
      ...
      peer.on('data', function(data) {
        ...
        if (dataObj.message) {
          ...
          if (dataObj.peerFilter === 'yes') {
            //applies filter
            document.getElementById('myVideo').style.filter = 'saturate(8)';
            //checks key value to see if a filter needs to be removed
          } else if (dataObj.peerFilter === 'no') {
            //removes filter
            document.getElementById('myVideo').removeAttribute('style');
          }
        }
      ...
      document.getElementById('myFilter').addEventListener('click', function() {
          ...
          if (!document.getElementById('myVideo').style.filter) {
      ...
      peer.on('stream', function(stream) {
        ...
        // document.getElementById('peerBooth').appendChild(video);
        video = document.getElementById('peerVideo')
        ...
        document.getElementById('peerBooth').appendChild(canvas);
