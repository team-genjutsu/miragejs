...
(function(window, undefined) {
  ...
  tracking.one = function(selector, opt_element) {
    //console.log("tracking one selector", selector);
        ...
        if (opt_options) {
          if (opt_options.camera) {
            if (!element.src) {
              ...
              //console.log("track video step 1", element);
  ...
  tracking.trackCanvas_ = function(element, tracker) {
    ...
    task.on('run', function() {
      //console.log("inside task on run for track canvas", element.width);
  ...
  tracking.trackVideo_ = function(element, tracker) {
      ...
      requestId = window.requestAnimationFrame(function() {
        if (element.readyState === element.HAVE_ENOUGH_DATA) {
          ...
          try {
            ...
            //console.log(element, "this is the eleent drawn on the context");
        ...
        //console.log("I just had to call request animation frame again", requestId);
        requestAnimationFrame_();
      });
      //console.log(requestId, "requestID", element.readyState);
    };
    ...
    task.on('run', function() {
      // console.log("task on run calling raf");
      requestAnimationFrame_();
    });
    return task.run();
  };
...
(function() {
  ...
  tracking.Image.computeIntegralImage = function(pixels, width, height, opt_integralImage, opt_integralImageSquare, opt_tiltedIntegralImage, opt_integralImageSobel) {
    //console.log("inside of tracking dot image compute intergral image", width, height);
    ...
    for (var i = 0; i < height; i++) {
      for (var j = 0; j < width; j++) {
        ...
        if (opt_integralImage) {
          ...
          //console.log("looking for intergral image inside of tracking image");
        }
        if (opt_integralImageSquare) {
          this.computePixelValueSAT_(opt_integralImageSquare, width, i, j, pixel * pixel);
          //console.log("looking for ingetral image square inside of tracking image");
        }
  ...
  tracking.Image.computePixelValueRSAT_ = function(RSAT, width, i, j, pixel, pixelAbove) {
    //console.log("inside compute pixel value rsat");
    var w = i * width + j;
  ...
  tracking.Image.computePixelValueSAT_ = function(SAT, width, i, j, pixel) {
    //console.log("ocmpute pixel value sat");
    var w = i * width + j;
  ...
  tracking.Image.sobel = function(pixels, width, height) {
    //console.log("inside tracking dot image dot sobel");
...
(function() {
  ...
  tracking.ViolaJones.detect = function(pixels, width, height, initialScale, scaleFactor, stepSize, edgesDensity, data) {
    ...
    //console.log('viola jones detect was called', data, width, height);
    ...
    while (blockWidth < width && blockHeight < height) {
      ...
      for (var i = 0; i < (height - blockHeight); i += step) {
        for (var j = 0; j < (width - blockWidth); j += step) {
          ...
          //console.log("eval stages exists!!!!!", this.evalStages_(data, integralImage, integralImageSquare, tiltedIntegralImage, i, j, width, blockWidth, blockHeight, scale));

          if (edgesDensity > 0) {
            if (this.isTriviallyExcluded(edgesDensity, integralImageSobel, i, j, width, blockWidth, blockHeight)) {
              //console.log(this.isTriviallyExcluded(edgesDensity, integralImageSobel, i, j, width, blockWidth, blockHeight));
              continue;
            }
          }
          //console.log("hello this takes place after the edgesDensity check");

          if (this.evalStages_(data, integralImage, integralImageSquare, tiltedIntegralImage, i, j, width, blockWidth, blockHeight, scale)) {
             //console.log("rects plus plus");
            rects[total++] = {
              width: blockWidth,
              height: blockHeight,
              x: j,
...
(function() {
  ...
  tracking.TrackerTask.prototype.run = function() {
    ...
    this.reemitTrackEvent_ = function(event) {
       //console.log("re emit track", event);
      self.emit('track', event);
    };
    this.tracker_.on('track', this.reemitTrackEvent_);
    //console.log("re emit run");
    this.emit('run');
    return this;
  };
  ...
  tracking.ObjectTracker.prototype.track = function(pixels, width, height) {
    ...
    classifiers.forEach(function(classifier) {
      results = results.concat(tracking.ViolaJones.detect(pixels, width, height, self.getInitialScale(), self.getScaleFactor(), self.getStepSize(), self.getEdgesDensity(), classifier));
       //console.log("inside the classifiers for each", results);
    });

    if(results[0]) {
       //console.log("emit track from object tracker dot track", results);
        this.emit('track', {
          data: results
      });
    } else {
